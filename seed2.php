...
foreach ($services as $s) {
    ...
    $check = $conn->query("SELECT idService FROM service WHERE nameService = '$name'");
    if ($check && $check->num_rows == 0) {
        $conn->query("INSERT INTO service (nameService, descriptionS, price, durationMonths, isActive) VALUES ('$name', '$desc', $price, 1, 1)");
    }
}
